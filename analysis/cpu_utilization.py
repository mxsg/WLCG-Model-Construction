...
def main():
    ...
    physical_util = pd.read_csv(os.path.join(directory, 'utilization-physical.csv'))
    physical_average = physical_util['utilization'].mean()
    logical_util = pd.read_csv(os.path.join(directory, 'utilization-logical.csv'))
    ...
    reference = [('efficiency from job reports (average 75.00%)', 0.75, 'C2'), ('efficiency from GridKa (average 67.21%)', 0.6721, 'C3')]
    ...
    axes.set_title("CPU utilization, demands from I/O ratio (May 2018)")

    fig.savefig(os.path.join(directory, 'utilizations_reference_comparison.pdf'))
