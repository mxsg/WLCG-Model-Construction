...
def visualize_walltimes(df: pd.DataFrame, output_path, output_name, bin_count=50, reference: pd.DataFrame = None):
    ...
    measured_mean = df['response_time'].mean()
    reference_mean = reference['walltime'].mean()

    print("Mean simulated response time: {}".format(measured_mean))
    print("Mean reference response time: {}".format(reference_mean))

    print("Deviation: {}".format((measured_mean - reference_mean)/reference_mean))
    ...
    for type, i_subplot in zip(jobtypes, range(len(jobtypes))):
        ...
        jobs_of_type_cutoff = jobs_of_type[jobs_of_type['response_time'] <= jobs_of_type['response_time'].quantile(0.95)]

        n, bins, patches = axes.hist(jobs_of_type_cutoff['response_time'], density=True, bins=bin_count, label='simulated',
                                     histtype='stepfilled')
    ...
    fig.savefig(os.path.join(output_path, output_name))
...
def main():
    ...
    walltime_filename = 'walltimes.csv'
    ...
    walltime_reference = pd.read_csv(os.path.join(base_path, config['walltimeReference']['path']))

    walltimes = {}
    for simulation_run in config['simulations']:
        walltime = pd.read_csv(os.path.join(base_path, simulation_run['path'], results_dir, walltime_filename))
        walltimes[simulation_run['key']] = (simulation_run, walltime)

        print("Visualizing {}".format(simulation_run['key']))
        visualize_walltimes(walltime, config['output'], 'walltimes_{}.pdf'.format(simulation_run['key']), bin_count=30, reference=walltime_reference)
